# KADA 多地区 Slot 架构设计（需求与设计核心要点）

> 本文档基于对当前代码的完整通读（非猜测），用于指导"按地区隔离出口"功能的实现。
> 目标：**在不动现有单节点行为的前提下，把"一个地区"抽象成可水平扩展的 Slot。**

> **实现状态（2026-08-02）**：阶段 1–3 已落地，采用**多进程按地区隔离**方案（而非阶段 3 原计划的进程内多线程）。
> 每个地区 = 一个独立 OS 进程，运行现有已验证的引擎，由 `slot_manager.py` 编排、`director.py` 提供管理界面；
> 出网隔离复用了项目已有的 `SO_BINDTODEVICE`（按 `VPNGATE_TUN_DEV` 把出向流量绑到本地区 tun 设备）机制。
> 单地区用户依旧直接 `python vpngate_manager.py`，行为完全不变。
